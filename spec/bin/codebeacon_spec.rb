...
RSpec.describe 'bin/codebeacon' do
  let(:codebeacon_script) { File.join(__dir__, '../../bin/codebeacon') }
  ...
  it 'can execute a Ruby script' do
    stdout, stderr, status = Open3.capture3(codebeacon_script, test_script)
    expect(status.success?).to be true
    expect(stdout).to include("Hello from test script")
  end

  it 'fails when no script is provided' do
    stdout, stderr, status = Open3.capture3(codebeacon_script)
